...
public:
    ...
    HashTable() {
        arr = new HashNode *[ARRAY_SIZE]; // dynamically creating array of pointers hash node to allocate fixed memory.
    ...
    void insertElement(int key, int value) {
        ...
        while(arr[h] != NULL && arr[h]->key != key) {  //This only when key different.
            h = hashFunction(h + 1);
        }
